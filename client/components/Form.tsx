...
//@ts-ignore
const Form = ({ title, accountLink, link }) => {
    const [email, setEmail] = useState('')
    const [password, setPassword] = useState('')



    const { doRequest, errors } = useRequest({ url: `/api/users/${link === "signin" ? "signup" : 'signin'}`, method: 'post', body: { email, password }, onSuccess: () => Router.push('/') })
